...
public class FortuneWheelManager : MonoBehaviour
{
	...
	{
		// define coin amount
		//1. _currentCoinsAmount = GameData.instance.GetPlayerCoin();
		_currentCoinsAmount = 100000;//(int)DataManager.playerData.money;//PlayerPrefs.GetInt("num_coins");
        // spin cost
        _turnCost = SpinCost;
	...
	private void TurnWheel (bool isFree)
	{
		...
		if (!isFree) {
            ...
            // reduce coin sound
            //2.  AudioManager.instance.CoinPayAudio();
            PlayerPrefs.SetInt("num_coins", PlayerPrefs.GetInt("num_coins") - _turnCost);
            // update game data
            //3.   GameData.instance.SavePlayerCoin(GameData.instance.GetPlayerCoin() - _turnCost);
            // update text label
            if (GameObject.Find("MapScene"))
            {
              //4.  GameObject.Find("MapScene").GetComponent<MapScene>().UpdateCoinAmountLabel();
            }
	...
	private void GiveAwardByAngle ()
	{
		...
		case -210:
            RewardCoins(Sector6);
                Debug.Log(" 5 Color Balls");
                break;
		// Sector 7
		case -180:
            RewardCoins(Sector7);
                Debug.Log(" 5 Lollipop");
                break;
		// Sector 8
		case -150:
            RewardCoins(Sector8);
    ...
    private void RewardCoins(LUCKY_SPIN_REWARDS award)
	{
        ...
        switch (award)
        {
            ...
            case LUCKY_SPIN_REWARDS.BEGIN_BOMB_BREAKER_REWARD:
                ...
                if (award == LUCKY_SPIN_REWARDS.THIRTY_COINS_REWARD) awardCoins = 5;
                else if (award == LUCKY_SPIN_REWARDS.SINGLE_BREAKER_REWARD) awardCoins = 10;
                else if (award == LUCKY_SPIN_REWARDS.COL_BREAKER_REWARD) awardCoins = 20;
                else if (award == LUCKY_SPIN_REWARDS.OVEN_BREAKER_REWARD) awardCoins = 50;
                else if (award == LUCKY_SPIN_REWARDS.FIVE_COINS_REWARD) awardCoins = 100;
                else if (award == LUCKY_SPIN_REWARDS.BEGIN_BOMB_BREAKER_REWARD) awardCoins = 500;
                ...
                // plus coin
                //5. GameData.instance.SavePlayerCoin(_currentCoinsAmount);
                PlayerPrefs.SetInt("num_coins", _currentCoinsAmount);
                // play add coin sound
                //6.  AudioManager.instance.CoinAddAudio();
                
                // update text label
                if (GameObject.Find("MapScene"))
                {
                   //7. GameObject.Find("MapScene").GetComponent<MapScene>().UpdateCoinAmountLabel();
                }
            ...
            case  LUCKY_SPIN_REWARDS.BEGIN_RAINBOW_REWARD:
                ...
                if (award == LUCKY_SPIN_REWARDS.TEN_COINS_REWARD)
                {
                    CoinsDeltaImage.overrideSprite = awardImage[2];
                    PlayerPrefs.SetInt("num_boosters_1", PlayerPrefs.GetInt("num_boosters_1") + 2);
                    DeltaCoinsText.text = "2";
                    //8.   GameData.instance.SaveSingleBreaker(GameData.instance.GetSingleBreaker() + boosterRewardCount);
                }
                else if (award == LUCKY_SPIN_REWARDS.ROW_BREAKER_REWARD)
                {
                    CoinsDeltaImage.overrideSprite = awardImage[1];
                    PlayerPrefs.SetInt("num_boosters_0", PlayerPrefs.GetInt("num_boosters_0") + 2);
                    DeltaCoinsText.text = "2";

                    //9.   GameData.instance.SaveRowBreaker(GameData.instance.GetRowBreaker() + boosterRewardCount);
                }
                else if (award == LUCKY_SPIN_REWARDS.COL_BREAKER_REWARD)
                {
                  //  CoinsDeltaImage.overrideSprite = awardImage[3];
                  //10.  GameData.instance.SaveColumnBreaker(GameData.instance.GetColumnBreaker() + boosterRewardCount);
                }
                else if (award == LUCKY_SPIN_REWARDS.RAINBOW_BREAKER_REWARD)
                {
                    CoinsDeltaImage.overrideSprite = awardImage[4];
                    PlayerPrefs.SetInt("num_boosters_3", PlayerPrefs.GetInt("num_boosters_3") + 5);
                    DeltaCoinsText.text = "5";
                    //11.  GameData.instance.SaveRainbowBreaker(GameData.instance.GetRainbowBreaker() + boosterRewardCount);
                }
                else if (award == LUCKY_SPIN_REWARDS.OVEN_BREAKER_REWARD) 
                {
                  //  CoinsDeltaImage.overrideSprite = awardImage[5];
                  //12.  GameData.instance.SaveOvenBreaker(GameData.instance.GetOvenBreaker() + boosterRewardCount);
                }
                else if (award == LUCKY_SPIN_REWARDS.BEGIN_FIVE_MOVE_REWARD) 
                {
                    CoinsDeltaImage.overrideSprite = awardImage[4];
                    PlayerPrefs.SetInt("num_boosters_3", PlayerPrefs.GetInt("num_boosters_3") + 2);
                    DeltaCoinsText.text = "2";
                    //13.   GameData.instance.SaveBeginFiveMoves(GameData.instance.GetBeginFiveMoves() + boosterRewardCount);
                }
                else if (award == LUCKY_SPIN_REWARDS.BEGIN_RAINBOW_REWARD)
                {
                    CoinsDeltaImage.overrideSprite = awardImage[3];
                    PlayerPrefs.SetInt("num_boosters_2", PlayerPrefs.GetInt("num_boosters_2") + 2);
                    DeltaCoinsText.text = "2";
                    //14.  GameData.instance.SaveBeginRainbow(GameData.instance.GetBeginRainbow() + boosterRewardCount);
                }
                else if (award == LUCKY_SPIN_REWARDS.TWENTY_COINS_REWARD)
                {
                    CoinsDeltaImage.overrideSprite = awardImage[1];
                    PlayerPrefs.SetInt("num_boosters_0", PlayerPrefs.GetInt("num_boosters_0") + 5);
                    DeltaCoinsText.text = "5";
                    //15.    GameData.instance.SaveBeginBombBreaker(GameData.instance.GetBeginBombBreaker() + boosterRewardCount);
                }

                // play add coin sound
              //16.  AudioManager.instance.CoinAddAudio();
    ...
    {
      //17.  AudioManager.instance.ButtonClickAudio();
    }
